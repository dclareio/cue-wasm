...
CueWasmAPI.toJSONSchema = (cueString) => {
  if (Array.isArray(cueString)) {
    cueString = cueString.map(cs => isString(cs) ? cs : JSON.stringify(cs));
    cueString = cueString.join('\n');
  } else if (!isString(cueString)) {
  ...
  const result = CueWasmAPI.toJSONSchemaImpl(cueString);
  console.log('******', cueString, result)
  if (result && result.error) throw result.error;
  return result.value.components.schemas;
};

CueWasmAPI.parseSchema = (cueString) => JSON.parse(CueWasmAPI.toJSONSchema(cueString));
CueWasmAPI.schema = (strings, ...values) => {
  // called when using the cue`` tagged template literal syntax
  const builtString = strings.map((s, i) => {
    let value = values[i];
    if (!value) return s;
  ...
  return CueWasmAPI.parseSchema(builtString)
}