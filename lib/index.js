import "./polyfill.js";
import isString from 'lodash.isstring';
import pako from "pako";
import toJsonSchema from '@openapi-contrib/openapi-schema-to-json-schema';

global.CueWasmAPI = {}

export const init = async () => {
  const variant = 'full';
  if (CueWasmAPI[variant] && CueWasmAPI[variant].loaded) return CueWasmAPI[variant];

  await import("./wasm_exec.full.cjs");
  const cueWasm = await import('./cue.wasm.full.inline.js');

  const cueBuff = Buffer.from(cueWasm.default, 'base64');
  const cueArray = new Uint8Array(cueBuff);
  const cueUnzipped = pako.ungzip(cueArray);
  const go = new Go();
  const { instance } = await WebAssembly.instantiate(cueUnzipped, go.importObject);
  go.run(instance);
  const cue = (strings, ...values) => {
    // called when using the cue`` tagged template literal syntax
    const builtString = strings.map((s, i) => {
      let value = values[i];
      if (!value) return s;
      // don't quote strings to allow users to dynamically write cue vs.
      // just injecting json values
      if (!isString(value)) value = JSON.stringify(value);
      return s + value;
    }).join('')
    return cue.parse(builtString)
  }

  // ensure we don't overwrite the go functions with future variants
  // memoize go functions as calls into go are expensive and these are pure functions
  cue.toJSONImpl = CueWasmAPI._toJSONImpl
  cue.toOpenAPIImpl = CueWasmAPI._toOpenAPIImpl
  cue.toASTImpl = CueWasmAPI._toASTImpl

  cue.toJSON = (cueString) => {
    if (Array.isArray(cueString)) {
      cueString = cueString.map(cs => isString(cs) ? cs : JSON.stringify(cs));
      cueString = cueString.join('\n');
    } else if (!isString(cueString)) {
      cueString = JSON.stringify(cueString)
    }
    const result = cue.toJSONImpl(cueString);
    if (result && result.error) throw result.error;
    return result.value;
  };

  cue.parse = (cueString) => JSON.parse(cue.toJSON(cueString));

  cue.toOpenAPI = (cueString) => {
    if (Array.isArray(cueString)) {
      cueString = cueString.map(cs => isString(cs) ? cs : JSON.stringify(cs));
      cueString = cueString.join('\n');
    } else if (!isString(cueString)) {
      cueString = JSON.stringify(cueString)
    }
    const result = cue.toOpenAPIImpl(cueString);
    if (result && result.error) throw result.error;
    return result.value;
  };

  cue.parseSchema = (cueString) => toJsonSchema(JSON.parse(cue.toOpenAPI(cueString)).components.schemas);
  cue.schema = (strings, ...values) => {
    // called when using the cue`` tagged template literal syntax
    const builtString = strings.map((s, i) => {
      let value = values[i];
      if (!value) return s;
      // don't quote strings to allow users to dynamically write cue vs.
      // just injecting json values
      if (!isString(value)) value = JSON.stringify(value);
      return s + value;
    }).join('')
    return cue.parseSchema(builtString)
  }

  cue.toAST = (cueString) => {
    if (Array.isArray(cueString)) {
      cueString = cueString.map(cs => isString(cs) ? cs : JSON.stringify(cs));
      cueString = cueString.join('\n');
    } else if (!isString(cueString)) {
      cueString = JSON.stringify(cueString)
    }
    const result = cue.toASTImpl(cueString);
    if (result && result.error) throw result.error;
    return result.value;
  };

  cue.parseAST = (cueString) => JSON.parse(cue.toAST(cueString));
  cue.ast = (strings, ...values) => {
    // called when using the cue`` tagged template literal syntax
    const builtString = strings.map((s, i) => {
      let value = values[i];
      if (!value) return s;
      // don't quote strings to allow users to dynamically write cue vs.
      // just injecting json values
      if (!isString(value)) value = JSON.stringify(value);
      return s + value;
    }).join('')
    return cue.parseAST(builtString)
  }
  CueWasmAPI[variant] = cue;
  // ensure later assignments to the global don't mess with this variant
  CueWasmAPI[variant].loaded = true;
  return cue;
};
